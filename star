-- ============================================================
-- STAR DUELS V3 - TARGET HUB STYLE (with Always-On Speed Display)
...
-- MODIFIED: Visual style to match TARGET HUB (grey/white/dark)
...
if introShouldPlay then
    ...
    -- SOUND ADDED HERE: play a sound when intro appears, will be destroyed with introGui
    ...
    logo.Image = LOGO_ID
    logo.ImageColor3 = Color3.fromRGB(180,185,190)
    ...
    sub.Text = "TASER HUB IN UR FACE ًںک‚"
    sub.TextColor3 = Color3.new(1,1,1) -- WHITE
    ...
    loadingBar.BackgroundColor3 = Color3.fromRGB(180,185,190)
...
-- ============================================================
-- TARGET HUB STYLE: COLORS AND NEON BORDER
-- ============================================================
...
-- FIXED: toggleBatV2 now properly handles GUI state
...
local function Main()
    ...
    subTitleLbl.Text = "USING TASER HUB"
    ...
    local function makeSectionHeader(label)
        ...
        dot.Position = UDim2.new(0,12,0.5,-1.5)
    ...
    -- ============================================================
    -- STEAL BAR WITH PING (TARGET HUB STYLE)
    -- ============================================================
    local stealBarFrame = Instance.new("Frame", gui)
    stealBarFrame.Name = "GreenDuelsStealBar"
    ...
    do
        local page = buildPage("Auto Steal", function()
            ...
            makeGap(4)
            makeSectionHeader("Steal Config")
            makeGap(1)
    ...
    -- Visual Page (Clean Effects removed)
    do
        local page = buildPage("Visual", function()
            ...
            makeGap(6)
            makeSectionHeader("Other Visuals")
            makeGap(1)
    ...
    do
        local page = buildPage("Settings", function()
            ...
            saveBtn.MouseEnter:Connect(function()
                TweenService:Create(saveBtn,TweenInfo.new(0.1),{BackgroundColor3=GREY_LIGHT}):Play()
            end)
            saveBtn.MouseLeave:Connect(function()
                TweenService:Create(saveBtn,TweenInfo.new(0.1),{BackgroundColor3=C.accent}):Play()
            end)
            saveBtn.MouseButton1Click:Connect(function()
                local success = pcall(saveConfig)
                if success then
                    saveBtn.Text="âœ“ Saved!"
                    saveBtn.BackgroundColor3=GREY_LIGHT
                else
                    saveBtn.Text="âœ— Failed"
                    saveBtn.BackgroundColor3=Color3.fromRGB(80,0,0)
                end
            ...
            resetBtn.Text="â†؛ Reset Positions"
            ...
            resetBtn.MouseButton1Click:Connect(function()
                ...
                task.delay(1.8,function()
                    if resetBtn and resetBtn.Parent then resetBtn.Text="â†؛ Reset Positions" end
                end)
            end)
            ...
            _G._VezyFlashSave = function(success)
                ...
                if success then
                    lbl.Text="âœ“ Auto-saved"
                    lbl.TextColor3=GREY
                else
                    lbl.Text="âœ— Save failed"
                    lbl.TextColor3=Color3.fromRGB(255,0,0)
                end
                task.delay(1.5,function()
                    if lbl and lbl.Parent then
                        lbl.Text="Taser Hub  آ·  ًں‘»"
        ...
        Conns.antiRag = RunService.Heartbeat:Connect(function()
            ...
            if isRag then
                for _, d in pairs(char:GetDescendants()) do
                    if d:IsA("BallSocketConstraint") or (d:IsA("Attachment") and string.find(d.Name, "RagdollAttachment")) then                        pcall(function() d:Destroy() end)
                    end
                end
    ...
    loadConfig()
    startAutoSteal()
    print("[Green Duels] Ready. Target Hub style with original Auto Steal!")
    print("[Green Duels] âœ“ Colors changed to grey/white/dark")
    print("[Green Duels] âœ“ Fonts changed to Bangers")
    print("[Green Duels] âœ“ Neon border effect added (rotating gradient)")
    print("[Green Duels] âœ“ Intro updated with Target Hub style")
    print("[Green Duels] âœ“ ESP persists after opponent respawn")
    print("[Green Duels] âœ“ Other Players Speed is always ON")
    print("[Green Duels] âœ“ Button press animation removed")
    print("[Green Duels] âœ“ Original Auto Steal (working version)")
    print("[Green Duels] âœ“ Auto steal properly saves state")
    print("[Green Duels] âœ“ Clean Effects button removed")
    print("[Green Duels] âœ“ Ping display added to steal bar (Cyber Hub style)")
    print("[Green Duels] âœ“ Steal bar size slightly reduced (270px)")
    print("[Green Duels] âœ“ Aimbot Follow Distance added to GUI")
    print("[Green Duels] âœ“ BAT V2 speed set to 59")
end
